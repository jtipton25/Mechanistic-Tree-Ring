...
dataSim <- simData(model = 'VS', s2_noise = 0.5, trend = TRUE)

process <- makeMCARTrend(dataSim$W[1:12, 447:556], 
...
Rcpp::sourceCpp('~/Mechanistic-Tree-Ring/mcmc/mcmcVS.cpp') ## loads the MCMC code to fit the model
...
start <- Sys.time()
out <- makeMCMC(dataSim$zeta * dataSim$H, dataSim$W[1:12, 447:556], 
                dataSim$W[13:24, 447:556], dataSim$species, params, process,
                sim = TRUE)
finish <- Sys.time()
finish - start
